...
3. İndirdiğiniz dosyayı çıkarın. İçindeki bir **`skills/`** klasörünün içinde iki klasör bulacaksınız:
...
### ⚡ En hızlısı — Claude Code ve diğer CLI'lar için tek satırlık kurulum

[Node.js](https://nodejs.org) kuruluysa, tek bir komut **her iki** beceriyi **Claude Code**'a (ve Gemini CLI, Cursor, Codex ve Agent Skills standardını izleyen diğer araçlara) ekler:

```bash
npx skills add aabuhammam-dh/telc-b1-coach
```

Ardından yapay zeka aracınızı yeniden başlatın; beceriler otomatik olarak yüklenir. *(Bu, Agent Skills ekosisteminden açık kaynaklı [`skills`](https://github.com/vercel-labs/skills) yükleyicisini kullanır — topluluk tarafından geliştirilir, resmi bir Anthropic aracı değildir.)*

**Claude Code'un yerleşik eklenti pazarını mı tercih edersiniz?** Bu depoyu bir pazar olarak ekleyin, ardından eklentiyi kurun:
...
Terminalle aranız yok mu ya da Claude web sitesini mi kullanıyorsunuz? Bunun yerine aşağıdaki bir seçeneği seçin.
...
2. İndirdiğinizin **`skills/`** klasörünün içinde bulunan **hem** `telc-b1-exam` **hem de** `telc-b1-schreiben` klasörlerini içine kopyalayın.
...
Her becerinin kendi kısa kılavuzu da vardır: [`skills/telc-b1-exam/README.md`](../skills/telc-b1-exam/README.md)
ve [`skills/telc-b1-schreiben/README.md`](../skills/telc-b1-schreiben/README.md).
...
- **Kutudan çıktığı gibi kullanıma hazırdır.** Beceriler başlangıç içeriğiyle (yaygın sınav tuzakları, örnek
  kalıplar, bir ifade bankası) gelir, bu yüzden hemen kullanışlıdırlar; siz pratik yaptıkça Claude kendini
  size göre uyarlar. Hiçbir kişisel veri içermez.
