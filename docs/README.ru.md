...
строгого и требовательного наставника для экзамена **telc Deutsch B1** (немецкий язык уровня B1). Этот наставник оценивает ваши тренировочные
...
- **`telc-b1-schreiben`** — помогает с **письменным заданием**: обучает формату, оценивает ваше
  письмо так, как это делают настоящие экзаменаторы, и отрабатывает те ошибки, которые вы повторяете снова и снова.

Вы просите нужное простым языком (*«оцени мои ответы»*, *«объясни разницу между weil и denn»*,
...
3. Распакуйте скачанный файл. Внутри папки `skills/` вы найдёте две папки с навыками:
...
### ⚡ Самый быстрый способ — установка одной командой для Claude Code и других CLI

Если у вас установлен [Node.js](https://nodejs.org), одна-единственная команда добавит **оба** навыка в **Claude Code** (а также в Gemini CLI, Cursor, Codex и другие инструменты, следующие стандарту Agent Skills):

```bash
npx skills add aabuhammam-dh/telc-b1-coach
```

После этого перезапустите ваш ИИ-инструмент — и навыки загрузятся автоматически. *(Здесь используется установщик с открытым исходным кодом [`skills`](https://github.com/vercel-labs/skills) из экосистемы Agent Skills — он поддерживается сообществом и не является официальным инструментом Anthropic.)*

**Предпочитаете встроенный маркетплейс плагинов Claude Code?** Добавьте этот репозиторий как маркетплейс, затем установите плагин:
...
Не любите работать в терминале или пользуетесь сайтом Claude? Тогда выберите один из вариантов ниже.
...
2. Скопируйте в неё **обе** папки `telc-b1-exam` и `telc-b1-schreiben` (они находятся внутри папки `skills/` в скачанном архиве).
...
|---|---|---|
| **`telc-b1-exam`** | Чтение, Sprachbausteine, аудирование + **устный** экзамен, оценивание, тренировки, грамматика, **создание оригинальных тренировочных тестов** и **обучение с проверкой по отдельным темам** с отслеживанием готовности | `[mock exam]`, `[topic "connectors"]`, `[log exam]`, «объясни разницу между obwohl и trotzdem» |
| **`telc-b1-schreiben`** | **Письменное задание** — формат, оценивание, отработка ошибок, обороты речи | `/written-grade`, «прошло бы это письмо?» |
...
У каждого навыка также есть своё короткое руководство: [`skills/telc-b1-exam/README.md`](../skills/telc-b1-exam/README.md)
и [`skills/telc-b1-schreiben/README.md`](../skills/telc-b1-schreiben/README.md).
...
| Помощь с письменным заданием по критериям telc |     ⚠️        |        ✅         |        ✅        |
